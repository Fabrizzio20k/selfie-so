...
uint64_t selfie_run_mipsterOS(uint64_t machine) {
  ...
  uint64_t counter_processes = 1;

  if (number_of_remaining_arguments() > 0) {
    if (string_compare(peek_argument(0), "-x")) {
      if (peek_argument(1) != 0) {
        counter_processes = atoi(peek_argument(1));
      }
    }
  }

  while (counter_processes > 0) {
    ...
    counter_processes = counter_processes - 1;
  }
    ...
    while (number_of_remaining_arguments() > 0) {
      ...
      if (string_compare(argument, "-c"))
        ...
        if (string_compare(argument, "-m")) {
          return selfie_run_mipsterOS(
              MIPSTER); // CAMBIADO POR selfie_run, para el task
